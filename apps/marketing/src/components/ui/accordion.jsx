...
const AccordionItem = React.forwardRef(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn('border border-border rounded-lg mb-2 px-4 bg-card', className)}
    {...props}
  />
));
...
const AccordionTrigger = React.forwardRef(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex flex-1 items-center justify-between py-5 text-left font-semibold text-lg transition-all hover:no-underline [&[data-state=open]>svg]:rotate-180',
        className
      )}
      {...props}
    >
